...
VLOCITY_BUILD_SALESFORCE_API_VERSION = '54.0';
...
DataPacksUtils.prototype.getExpandedDefinition = function(dataPackType, SObjectType, dataKey) {
    ...
    if (this.vlocity.isOmniStudioInstalled) {
        if (dataPackType && this.vlocity.omnistudio.platformObjectMappings[dataPackType]) {
            if (SObjectType) {
                if (this.vlocity.omnistudio.platformObjectMappings[dataPackType][SObjectType]) {
                    definitionValue = this.vlocity.omnistudio.platformObjectMappings[dataPackType][SObjectType][dataKey]; 
                }
            } else {
                definitionValue = this.vlocity.omnistudio.platformObjectMappings[dataPackType][dataKey]; 
            }
        }
    }
...
DataPacksUtils.prototype.addPackageDirToSfdxProjectJson = function(packageDir) {
    const sfdxProjectJsonPath = path.join('sfdx-project.json');
    let sfdxProjectJson = {
        "packageDirectories": [
            {
                "path": packageDir,
    ...
    fs.outputFileSync(sfdxProjectJsonPath, JSON.stringify(sfdxProjectJson), { encoding: 'utf8' });
}
...
DataPacksUtils.prototype.hasOnlyProductBaseInfo = function (productObject) {
    
    for (var key in productObject) {
        if (typeof productObject[key] === "object") {
            return false;
        }
    }
